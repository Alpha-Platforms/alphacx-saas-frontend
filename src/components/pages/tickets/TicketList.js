...
<<<<<<< HEAD
  const tableColumns = [
    ...
    },
    {
      ...
      render: (rowData) => (
        <div
          className={`ticket-state ${getStatusColor(rowData.status.status)}`}
        >
    ...
    const tableColumns = [
        {
            ...
            field: 'ticketId',
            width: '40%',
            render: rowData => <Link to={`/tickets/${rowData.ticketId}`} style={{ textTransform: 'uppercase' }}>{rowData.ticketId.slice(-8)}</Link>
        }, {
            title: 'Subject',
            field: 'subject',
            width: '40%'
...
>>>>>>> 08d811cd2604e5a5d56bb7b3f3a13a1d093b0735
