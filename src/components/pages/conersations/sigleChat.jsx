...
import { Swap, AddToChat, UserChatIcon, CheclkChat } from '../../../assets/images/svgs';
...
export default function SigleChat({
    ...
    return (
        <div>
                ...
                <div className="singleChat-Sender-img" style={{ position: 'relative' }}>
                    {SenderInfo?.customer?.avatar ? (
                        ...
                        <div className="singleChatSenderImg">
                            <p>{`${SenderInfo?.customer?.firstname?.slice(0, 1)}${
                                SenderInfo?.lastname == 'default' ? '' : SenderInfo?.customer?.lastname?.slice(0, 1)
                            }`}</p>
                        </div>
                    )}
                    <div className="single-chat-user-name">
                        <p>{`${capitalizeFirstLetter(SenderInfo?.customer?.firstname)} ${capitalizeFirstLetter(
                            SenderInfo?.customer?.lastname,
                        )}`}</p>
                        <p>{`Via ${ticket[0].channel} (${dateFormater(ticket[0].updated_at)})`}</p>
                    </div>
                </div>
                <div className="alignt-action-right-single">
                    <div className="action-on-d-single-chat">
                        <select name="" id="" onChange={(e) => upTicketStatus(e.target.value)}>
                    ...
                    <div className="single-chat-swap-icon" onClick={() => setshowUserProfile(true)}>
                        <UserChatIcon />
                    </div>

                    <div className="single-chat-swap-icon" onClick={() => setopenSaveTicketModal(!openSaveTicketModal)}>
                        <CheclkChat />
                    </div>
                </div>
            </div>
            ...
            <div className="singleChat-line-br" />
            <div className="chats-body">
                <div className="sender-full-message-body">{ticket[0]?.plain_description}</div>
                <div className="msgTime-single">{dateFormater(ticket[0].updated_at)}</div>

                <div className="siglechat-hr" />
                {noResponseFound ? (
                    <p
                        style={{
                            textAlign: 'center',
                                        ...
                                        <div className="singleChat-Sender-img" style={{ position: 'relative' }}>
                                            {e?.user?.avatar == null ? (
                                                <div className="singleChat-Sender-imgNoImg">
                                                    ...
                                                    )} ${capitalizeFirstLetter(e?.user?.lastname?.slice(0, 1))}`}</p>
                                                </div>
                                            ) : (
                                            ...
                                            <div className="single-chat-user-name">
                                                ...
                                                <p>{`Via ${ticket[0].channel} (${dateFormater(e?.created_at)})`}</p>
                                            </div>
                                        </div>
                                        <div className="single-chat-response">
                                            <div dangerouslySetInnerHTML={createMarkup(e?.response)} />
                                        </div>
                                    </div>
                                );
                            });
