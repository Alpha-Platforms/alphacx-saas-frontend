...
export default function TicketTimeline({ ticket, UserInfo, isTicketDetails, timeLine = true }) {
  ...
  const ticketAttachments = [ticket[0]?.attachment].filter(x => x ? true : false).map(file => {
    let fileSplit = file.split('.');
    let slashSplit = file.split('/');
    return {
      link: file,
  ...
  const ticketImageAttachments = ticketAttachments.filter(attach => attach.ext === "jpg" || attach.ext === "jpeg" || attach.ext === "png" || attach.ext === "gif").map(imgObj => imgObj?.link);
      ...
      <div className='ticket-attachment'>
        ...
        <ul className="attach-wrapper">
          {
            ticketAttachments.map(attach => {
              switch(attach?.ext) {
                ...
                case 'gif':
                  return <li>
                  <a href="#" onClick={() => setLightboxState(prev => ({...prev, isOpen: true, idx: ticketImageAttachments.indexOf(attach?.link),}))}><div><TypeImg /></div>
                  <div>{attach.name}</div></a>
                  </li>
                case 'pdf':
                  return <li><a href={attach?.link} target="_blank" rel="noopener noreferrer" ><div><TypePdf/></div><div>{attach.name}</div></a></li>
                case 'csv':
                  return <li><a href={attach?.link} target="_blank" rel="noopener noreferrer" download={attach.name}><div><TypeCsv/></div><div>{attach.name}</div></a></li>
                case 'docx':
                case 'doc':
                  return <li><a href={attach?.link} target="_blank" rel="noopener noreferrer" download={attach.name}><div><TypeDocx/></div><div>{attach.name}</div></a></li>
                case 'xlsx':
                case 'xls':
                  return <li><a href={attach?.link} target="_blank" rel="noopener noreferrer" download={attach.name}><div><TypeXlsx/></div><div>{attach.name}</div></a></li>
                default:
                  return <li><a href={attach?.link} target="_blank" rel="noopener noreferrer" download={attach.name}><div><TypePdf/></div><div>{attach.name}</div></a></li>
              }
            })
          }
        </ul>
        ...
        {lightboxState.isOpen && (
          <Lightbox
            ...
            prevSrc={ticketImageAttachments[(lightboxState.idx + ticketImageAttachments.length - 1) % ticketImageAttachments.length]}
            onCloseRequest={() => setLightboxState(prev => ({...prev, isOpen: false }))}
            onMovePrevRequest={() => setLightboxState(prev => ({
                ...prev,
                idx: (lightboxState.idx + ticketImageAttachments.length - 1) % ticketImageAttachments.length,
              }))
            }
            onMoveNextRequest={() => setLightboxState(prev => ({
                ...prev,
                idx: (lightboxState.idx + 1) % ticketImageAttachments.length,
              }))
            }
            imageCaption={ticketAttachments.find(img => img.link === ticketImageAttachments[lightboxState.idx])?.name}
            enableZoom={true}
          />
        )}
