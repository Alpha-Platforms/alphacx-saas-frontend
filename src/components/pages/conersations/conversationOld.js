...
export default function Conversation() {
    ...
    const filterTicket = (value, type) => {
        if (type == 'channel') {
            AppSocket.createConnection();
            const data = { channel: value, per_page: 100 };
            AppSocket.io.emit(`ws_tickets`, data);
        }

        if (type == 'status') {
            AppSocket.createConnection();
            const data = { status: value, per_page: 100 };
            AppSocket.io.emit(`ws_tickets`, data);
        }
    };
    ...
    const upTicketStatus = async (id) => {
        ...
        if (res.status == 'success') {
            // setStatues(res?.data?.statuses);
            return NotificationManager.success('Ticket status update successfully', 'Success', 4000);
        }
        return NotificationManager.error(res.er.message, 'Error', 4000);
    };
    ...
    return (
        <div className="conversation-wrap codei-ui-andy-setDefaults">
            <div className="conversation-layout">
                ...
                >
                    {loadSingleTicket ? (
                        ...
                        >
                            {' '}
                            <ClipLoader color="#0d4166" loading={loadSingleTicket} size={35} />
                        </div>
                    ) : firstTimeLoad ? (
                        ...
                        <div className="single-chat-ckeditor">
                            ...
                            <div className="sendMsg">
                                <button disabled={sendingReply} onClick={() => replyTicket(ReplyTicket, 'attachment')}>
                                    Send
                                </button>
                            </div>
                        </div>
            ...
            <div style={showUserProfile ? { display: 'block' } : { display: 'none' }}>
                <UserProfile setshowUserProfile={setshowUserProfile} UserInfo={UserInfo} />
            </div>
            <div>
                <Modal open={openSaveTicketModal} onClose={closeSaveTicketModal} center>
                    <div className="saveTicketWrapModal">
                        <div className="modalHeaderSaveT">Kindly update ticket before closing the chat</div>
                        ...
                        <div className="saveTicketModalForm">
                            ...
                            <div className="closeTicketModdalj">
                                <select name="" id="" onChange={(e) => updateTicket(e.target.value)}>
