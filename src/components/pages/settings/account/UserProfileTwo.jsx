...
function UserProfileTwo({
    ...
    useEffect(() => {
        setSelectedRole({ value: currentAgent?.role, label: currentAgent?.role });
    }, [currentAgent]);
    ...
    useEffect(() => {
        if (currentAgent) {
            ...
            if (currentAgent?.role === 'Administrator') {
                if (admins) {
                    otherRolesTeamId = admins.filter((admin) => admin?.id === id)[0]?.group_id;
                }
            } else if (currentAgent?.role === 'Supervisor') {
                if (supervisors) {
                    otherRolesTeamId = supervisors.filter((supervisor) => supervisor?.id === id)[0]?.group_id;
                }
            }

            const { firstname, lastname, email, role, avatar, group_id, groups } = currentAgent;

            const agentInfo = group_id
                ? { firstname, lastname, email, role, team: group_id }
                : otherRolesTeamId
                ? { firstname, lastname, email, role, team: otherRolesTeamId }
            ...
            if (Array.isArray(groups)) {
                ...
                setSelectedTeams(
                    groups?.map((group) => ({ label: group?.group?.name || '', value: group?.group?.id || '' })),
                );
            }
        }
                                ...
                                <div className="d-flex mb-3">
                                    <div className="me-2 w-100">
                                        ...
                                        <RSelect
                                            ...
                                            name="role"
                                            defaultValue={RSRoles.filter(
                                                (option) => option.value === currentAgent.role,
                                            )}
                                            options={RSRoles}
                                            onChange={(options) => {
                                    ...
                                    <div className="w-100">
                                        ...
                                        <RSelect
                                            ...
                                            name="teams"
                                            value={selectedTeams}
                                            defaultValue={RSTeams.filter(
                                                (option) => option.value === currentAgent.group_id,
                                            )}
                                            options={RSTeams}
                                            isSearchable
                                            placeholder=""
                            ...
                            <div>
                                <div className="d-flex mb-4 mt-3">
                                    ...
                                    >
                                        ...
                                        >
                                            {uploadInfo.blob || uploadInfo.ownAvatar ? (
                                                <img
                                                    className="avatarImage"
                                                    src={uploadInfo.ownAvatar ? uploadInfo.ownAvatar : uploadInfo.blob}
                                                    alt=""
                                                    onLoad={() =>
                                                        uploadInfo.blob && URL.revokeObjectURL(uploadInfo.blob)
                                                    }
                                                    style={{
                                                        maxWidth: '100%',
                                                        maxHeight: '100%',
                                    ...
                                    <div>
                                        ...
                                        <p className="uploadInfoWrapper">
                                            <small
                                                id="uploadPersonalPhotoInputInfo"
                                                className={`${uploadInfo.error && 'text-danger'}`}
                                            >
                                                {uploadInfo.msg}
                                            </small>
                                        </p>
