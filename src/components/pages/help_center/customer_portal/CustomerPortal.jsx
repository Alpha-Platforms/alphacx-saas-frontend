...
function TicketPagination({ currentPage, numberOfPages, handleTicketPagination }) {
    const pageArr = [];
                ...
                case numberOfPages:
                    pageArr.push(
                        <li className="page-item">
                            <span className="page-link" onClick={() => handleTicketPagination(page)}>
                                {page}
                            </span>
                        </li>,
                    );
        ...
        <nav aria-label="ticket pagination">
            <ul className="pagination">
                <li className={`page-item ${currentPage <= 1 ? 'disabled' : ''}`}>
                    <span
                        className="page-link"
                        onClick={() => currentPage > 1 && handleTicketPagination(currentPage - 1)}
                    >
                        Previous
                    </span>
                </li>
                ...
                <li className={`page-item ${currentPage >= numberOfPages ? 'disabled' : ''}`}>
                    <span
                        className="page-link"
                        onClick={() => currentPage < numberOfPages && handleTicketPagination(currentPage + 1)}
                    >
                        Next
                    </span>
                </li>
...
function CustomerPortal({ statuses, categories }) {
    const emptyArea = [1, 2];
    ...
    return (
        <>
            <HelpNavBar activeBG custId={custId} />
            <TopBar />
            <div className="customer-portal">
                ...
                <div className="tickets-container">
                    <div className="tickets-list">
                        <div className="top">
                            ...
                            <div className="statuses">
                                <div className="tag">
                                    <p
                                        className={`all ${custState.currentStatus === '' ? 'active' : ''}`}
                                        ...
                                        <div className="tag">
                                            <p
                                                className={`${getStatusColor(status?.id)} ${
                                                    custState.currentStatus === status?.id ? 'active' : ''
                                                }`}
                        ...
                        {!custState.isCustTicketsLoaded ? (
                            ...
                            custState.custTickets.map((ticket, i) => (
                                <TicketItem key={i} ticket={ticket} getStatusColor={getStatusColor} />
                            ))
                        )}
                        <div className="mt-4">
                            {custState.isCustTicketsLoaded && (
                                <div>Showing 1-10 of {custState.meta?.totalPages} entries</div>
                            )}
                            <div className="float-end mt-2">
                                {custState.isCustTicketsLoaded && (
                                    <TicketPagination
                                        numberOfPages={custState.meta?.totalPages || 1}
                                        currentPage={custState.meta?.currentPage || 1}
                                        handleTicketPagination={handleTicketPagination}
                                    />
                                )}
                            </div>
                        </div>
                    ...
                    <div className="submit-ticket">
                        ...
                        <p>Providing these details will help us resolve your question faster.</p>
