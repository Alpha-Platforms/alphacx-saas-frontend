...
// ticket tables
const Tickets = ({ tickets, meta, handleFilterApply, tableColumns, handleSelectionChange }) => {
  ...
  return (
    <div className="reports-filter-wrapper">
      ...
      <div>
        ...
        {filters.map((item) => <span key={item?.uuid} onClick={() => setFilters((prev) => prev.filter((x) => x?.uuid !== item?.uuid))} style={{ color: item?.color, background: `${item?.color}10` }}>{ item?.label } &nbsp;&nbsp;&nbsp;×</span>)}
      ...
      <div className={`reports-filter-body ${ticketData.tickets ? 'active': ''}`}>
        {ticketData?.tickets && (<>
          ...
          <div className="ticket-table-wrapper">
            <Tickets handleFilterApply={handleFilterApply} tickets={ticketData.tickets} meta={ticketData.meta} tableColumns={tableColumns} handleSelectionChange={handleSelectionChange} />
          </div>
        </>)}
      </div>
      {(loading) && (
