...
export function AuthProvider(props) {
    ...
    const [userAvatar, setUserAvatar] = useState('https://tooxclusive.com/wp-content/uploads/2019/12/simi.jpg');
    ...
    const GetUserInfoFromStorge = () => {
        ...
        if (userData) {
            ...
            const wallet = parseUserData.accounts.filter((data) => {
                return data.acctype === 'Wallet';
            });
            setUserWallet(wallet);
            const clearingaccount = parseUserData.accounts.filter((data) => {
                return data.acctype === 'clearingaccount';
            });
            setUserClearingAccount(clearingaccount);

            const getUserAccounts = parseUserData.accounts.filter((data) => {
                return data.acctype !== 'clearingaccount';
            });
