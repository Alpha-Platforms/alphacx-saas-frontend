...
export const getUsers = () => (dispatch, getState) => {
    ...
    axios
        .get(`${config.stagingBaseUrl}/users`, userTokenConfig(getState))
        .then((res) =>
            dispatch({
...
export const getPaginatedUsers = (itemsPerPage, currentPage) => (dispatch, getState) => {
    ...
    dispatch(setUsersLoading());
    axios
        .get(`${config.stagingBaseUrl}/users?per_page=${itemsPerPage}&page=${currentPage}`, userTokenConfig(getState))
        .then((res) =>
            dispatch({
                type: types.GET_USERS,
                payload: res.data && res.data.status === 'success' ? res.data.data : {},
...
export const getSearchedUsers = (itemsPerPage, currentPage, searchVal) => (dispatch, getState) => {
    ...
    axios
        .get(
            `${config.stagingBaseUrl}/users?per_page=${itemsPerPage}&page=${currentPage}&search=${searchStr}`,
...
export const updateUser = async (updatedUser) => {
    ...
    try {
        const res = await axios.patch(
            `${config.stagingBaseUrl}/users/${updatedUser.id}`,
            body,
            userTokenConfig(getState),
        );
        return res.data;
