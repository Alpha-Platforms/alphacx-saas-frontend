...
export const getCurrentCustomer = (id, refetchCust) => (dispatch, getState) => {
    ...
    const currentCustomer = customers.filter((customer) => customer?.id === id)[0];
    ...
    } else if (getState().customer.currentCustomer?.id === id) {
        dispatch({
            type: types.GET_CURRENT_CUSTOMER,
...
export const getPaginatedCurrentCustomerTickets = (itemsPerPage, currentPage, customerId) => (dispatch, getState) => {
    ...
    axios
        .get(
            `${config.stagingBaseUrl}/customer/${customerId}/tickets?per_page=${itemsPerPage}&page=${currentPage}`,
            userTokenConfig(getState),
        )
        .then((res) =>
            dispatch({
...
export const getCustTickets = async (itemsPerPage, currentPage, customerId, statusId) => {
    ...
    }/customer/${customerId}/tickets?per_page=${itemsPerPage}&page=${currentPage}${
