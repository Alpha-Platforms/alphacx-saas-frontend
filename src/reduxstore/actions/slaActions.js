...
export const getSlas = () => (dispatch, getState) => {
    ...
    axios
        .get(`${config.stagingBaseUrl}/sla?per_page=50`, userTokenConfig(getState))
        .then((res) =>
            dispatch({
                type: types.GET_SLAS,
...
export const addSla = (newSla, successCallback, failureCallback) => (dispatch, getState) => {
    ...
    axios
        .post(`${config.stagingBaseUrl}/sla`, body, userTokenConfig(getState))
        .then((res) => {
            dispatch({
                type: types.ADD_SLA,
                payload: res.data,
...
export const updateSla = (slaId, newSla, successCallback, failureCallback) => (dispatch, getState) => {
    ...
    axios
        .patch(`${config.stagingBaseUrl}/sla/${slaId}`, body, userTokenConfig(getState))
        .then((res) => {
            dispatch({
