...
export const getObservers = () => (dispatch, getState) => {
    ...
    axios
        .get(`${config.stagingBaseUrl}/users?role=Observer&per_page=500`, userTokenConfig(getState))
        .then((res) =>
            dispatch({
...
export const getPaginatedObservers = (itemsPerPage, currentPage) => (dispatch, getState) => {
    ...
    axios
        .get(
            `${config.stagingBaseUrl}/users?role=Observer&per_page=${itemsPerPage}&page=${currentPage}`,
